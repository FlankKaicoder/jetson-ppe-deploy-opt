...
if [[ $# -ne 4 ]]; then
  echo "usage: $0 benchmark|profile baseline|A|B file|camera frame_count" >&2
  exit 2
fi
run_kind="$1"
variant="$2"
...
[[ "${variant}" == "baseline" || "${variant}" == "A" || "${variant}" == "B" ]] || exit 2
...
run_id="exp15_${run_kind}_${variant}_${source_type}_${timestamp}"
...
for path in "${binary}" "${engine}" "${repo_root}/tools/exp15_validate.py"; do
  [[ -e "${path}" ]] || { printf 'result=FAIL missing=%s\n' "${path}" > "${output_dir}/failure_summary.txt"; exit 1; }
done
...
printf '%q ' "${command[@]}" > "${output_dir}/app_command.txt"; printf '\n' >> "${output_dir}/app_command.txt"
if [[ "${run_kind}" == "profile" ]]; then
  full_command=(nsys profile --trace=cuda,nvtx,osrt --sample=none --cpuctxsw=none
    --force-overwrite=true --output="${output_dir}/report" "${command[@]}")
else
  full_command=("${command[@]}")
fi
printf '%q ' "${full_command[@]}" > "${output_dir}/command.txt"; printf '\n' >> "${output_dir}/command.txt"
"${full_command[@]}" 2>&1 | tee "${output_dir}/run.log"
rc=${PIPESTATUS[0]}
printf '%s\n' "${rc}" > "${output_dir}/return_code.txt"
[[ ${rc} -eq 0 ]] || { printf 'result=FAIL return_code=%s\n' "${rc}" > "${output_dir}/failure_summary.txt"; exit "${rc}"; }
...
[[ ${validation_rc} -eq 0 ]] || { printf 'result=FAIL validation_rc=%s\n' "${validation_rc}" > "${output_dir}/failure_summary.txt"; exit 1; }
if [[ "${run_kind}" == "profile" ]]; then
  [[ -s "${output_dir}/report.nsys-rep" ]] || exit 1
  nsys stats --force-export=true "${output_dir}/report.nsys-rep" \
    > "${output_dir}/nsys_stats.txt" 2>&1
  printf '%s\n' "$?" > "${output_dir}/stats_return_code.txt"
fi
