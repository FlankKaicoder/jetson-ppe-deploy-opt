from __future__ import annotations

import argparse
import csv
import json
import math
import time
from pathlib import Path
from typing import Any

import torch.nn as nn
from ultralytics import YOLO

try:
    from ultralytics.utils.torch_utils import de_parallel
except ImportError:
    def de_parallel(model: nn.Module) -> nn.Module:
        return getattr(model, "module", model)

from models.blocks.attention_block import count_p3_attention_wrappers
from models.exp05_trainer import (
    FOCAL_ALPHA,
    FOCAL_GAMMA,
    ElementwiseFocalBCE,
    FocalDetectionLoss,
    FocalDetectionModel,
    FocalDetectionTrainer,
)


def load_last_row(path: Path) -> dict[str, Any]:
    with path.open("r", encoding="utf-8", newline="") as handle:
        rows = list(csv.DictReader(handle))
    if not rows:
        raise RuntimeError("results.csv contains no rows")
    result: dict[str, Any] = {}
    for key, value in rows[-1].items():
        key = key.strip()
        value = value.strip() if value is not None else ""
        try:
            result[key] = float(value)
        except ValueError:
            result[key] = value
    return result


def finite_metrics(row: dict[str, Any]) -> bool:
    keys = (
        "train/box_loss",
        "train/cls_loss",
        "train/dfl_loss",
        "metrics/mAP50(B)",
        "metrics/mAP50-95(B)",
    )
    return all(
        isinstance(row.get(key), (int, float)) and math.isfinite(float(row[key]))
        for key in keys
    )


def criterion_details(model: nn.Module) -> dict[str, Any]:
    criterion = getattr(model, "criterion", None)
    if criterion is None:
        criterion = model.init_criterion()
    bce = getattr(criterion, "bce", None)
    return {
        "model_class": f"{model.__class__.__module__}.{model.__class__.__name__}",
        "criterion_class": f"{criterion.__class__.__module__}.{criterion.__class__.__name__}",
        "classification_loss_class": f"{bce.__class__.__module__}.{bce.__class__.__name__}",
        "gamma": getattr(bce, "gamma", None),
        "alpha": getattr(bce, "alpha", None),
        "reduction": getattr(bce, "reduction", None),
    }


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--pretrained", required=True)
    parser.add_argument("--data", required=True)
    parser.add_argument("--output-root", required=True)
    parser.add_argument("--run-name", required=True)
    parser.add_argument("--report-dir", required=True)
    args = parser.parse_args()

    report_dir = Path(args.report_dir).resolve()
    report_dir.mkdir(parents=True, exist_ok=True)
    train_arguments = {
        "model": str(Path(args.pretrained).resolve()),
        "data": str(Path(args.data).resolve()),
        "epochs": 2,
        "imgsz": 640,
        "batch": 16,
        "device": 0,
        "workers": 8,
        "seed": 42,
        "deterministic": True,
        "optimizer": "AdamW",
        "lr0": 0.0015,
        "lrf": 0.01,
        "momentum": 0.9,
        "weight_decay": 0.0005,
        "warmup_epochs": 3.0,
        "amp": True,
        "cache": False,
        "patience": 2,
        "val": True,
        "save": True,
        "plots": False,
        "project": str(Path(args.output_root).resolve()),
        "name": args.run_name,
        "exist_ok": False,
        "verbose": True,
    }
    (report_dir / "train_arguments.json").write_text(
        json.dumps(train_arguments, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )

    start = time.perf_counter()
    trainer = FocalDetectionTrainer(overrides=train_arguments)
    trainer.train()
    elapsed_seconds = time.perf_counter() - start

    train_dir = Path(trainer.save_dir).resolve()
    best_pt = train_dir / "weights" / "best.pt"
    last_pt = train_dir / "weights" / "last.pt"
    results_csv = train_dir / "results.csv"
    artifacts = {
        "best_pt": best_pt.is_file(),
        "last_pt": last_pt.is_file(),
        "results_csv": results_csv.is_file(),
    }
    if not all(artifacts.values()):
        raise RuntimeError(f"missing smoke artifacts: {artifacts}")

    row = load_last_row(results_csv)
    trainer_model = de_parallel(trainer.model)
    ema_model = de_parallel(trainer.ema.ema)
    checkpoint_model = YOLO(str(best_pt)).model
    trainer_criterion = criterion_details(trainer_model)
    checkpoint_criterion = criterion_details(checkpoint_model)
    checks = {
        "artifacts": all(artifacts.values()),
        "finite_metrics": finite_metrics(row),
        "trainer_model_class": isinstance(trainer_model, FocalDetectionModel),
        "trainer_criterion": isinstance(trainer_model.criterion, FocalDetectionLoss),
        "trainer_focal_bce": isinstance(trainer_model.criterion.bce, ElementwiseFocalBCE),
        "checkpoint_model_class": isinstance(checkpoint_model, FocalDetectionModel),
        "checkpoint_focal_bce": isinstance(checkpoint_model.init_criterion().bce, ElementwiseFocalBCE),
        "no_attention_trainer": count_p3_attention_wrappers(trainer_model) == 0,
        "no_attention_ema": count_p3_attention_wrappers(ema_model) == 0,
        "no_attention_checkpoint": count_p3_attention_wrappers(checkpoint_model) == 0,
    }
    overall = "PASS" if all(checks.values()) else "FAIL"
    summary = {
        "overall": overall,
        "run_name": args.run_name,
        "elapsed_seconds": elapsed_seconds,
        "train_dir": str(train_dir),
        "best_pt": str(best_pt),
        "checks": checks,
        "last_metrics": row,
        "trainer_criterion": trainer_criterion,
        "checkpoint_criterion": checkpoint_criterion,
        "parameters": sum(parameter.numel() for parameter in checkpoint_model.parameters()),
        "focal_gamma": FOCAL_GAMMA,
        "focal_alpha": FOCAL_ALPHA,
    }
    (report_dir / "summary.json").write_text(
        json.dumps(summary, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )
    lines = [
        "Exp05.2 Focal Classification Loss Smoke Training",
        f"overall={overall}",
        f"run_name={args.run_name}",
        f"elapsed_seconds={elapsed_seconds:.3f}",
        f"best_pt={best_pt}",
        f"parameters={summary['parameters']}",
        f"checks={json.dumps(checks, sort_keys=True)}",
        f"last_mAP50={row.get('metrics/mAP50(B)')}",
        f"last_mAP50_95={row.get('metrics/mAP50-95(B)')}",
        f"trainer_criterion={json.dumps(trainer_criterion, sort_keys=True)}",
        f"checkpoint_criterion={json.dumps(checkpoint_criterion, sort_keys=True)}",
    ]
    (report_dir / "summary.txt").write_text("\n".join(lines) + "\n", encoding="utf-8")
    print("\n".join(lines))
    return 0 if overall == "PASS" else 1


if __name__ == "__main__":
    raise SystemExit(main())
