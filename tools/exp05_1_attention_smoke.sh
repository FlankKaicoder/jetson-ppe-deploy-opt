...
run_name="${1:-exp05_1_attention_smoke_$(date +%Y%m%d_%H%M%S)}"
...
echo "experiment=Exp05.1 attention smoke"
echo "run_name=$run_name"
echo "report_dir=$report_dir"

"$python_bin" -u tools/exp05_1_attention_smoke.py \
...
if [ "$python_return_code" -eq 0 ]; then
    echo "exp05_1_attention_smoke=PASS"
else
    echo "exp05_1_attention_smoke=FAIL"
fi
