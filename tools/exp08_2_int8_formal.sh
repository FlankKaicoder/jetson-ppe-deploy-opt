...
run_name="exp08_2_int8_formal_${timestamp}"
report_dir="$repo_dir/results/int8/$run_name"
artifact_dir="$artifact_root/$run_name"
engine_path="$artifact_dir/yolo11n_baseline_exp08_b1_640_int8.engine"
cache_path="$artifact_dir/yolo11n_baseline_exp08_int8.cache"
...
[ "$(find "$calibration_root/calibration/images" -maxdepth 1 -type f | wc -l)" -eq 256 ] || fail_early "calibration image count mismatch"
...
{
    echo "experiment=Exp08.2 formal INT8 build"
    ...
    echo "calibration_images=256_TRAIN_ONLY"
    ...
    echo "cache_reuse=false"
} > "$report_dir/environment.txt" 2>&1

{
    echo "$python_bin tools/exp08_build_int8.py --batch-size 1 --limit 0"
    echo "$python_bin tools/exp08_int8_smoke.py --imgsz 640 --confidence 0.25 --nms-iou 0.70"
    echo "Formal build uses all 256 train-only images and a fresh cache"
    echo "Build gate: engine/cache exist and summaries PASS"
    echo "Execution gate: output [1,7,8400] and finite"
...

{
    ...
    if [ -s "$engine_path" ]; then
        echo "engine_bytes=$(stat -c %s "$engine_path")"
        echo "engine_sha256=$(sha256sum "$engine_path" | cut -d ' ' -f 1)"
    fi
    if [ -s "$cache_path" ]; then
        echo "cache_bytes=$(stat -c %s "$cache_path")"
