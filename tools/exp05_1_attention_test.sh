...
weights="/root/autodl-tmp/jetson-ppe-outputs/exp05_1_attention_e100_20260805_155714/weights/best.pt"
...
run_name="${1:-exp05_1_attention_test_$(date +%Y%m%d_%H%M%S)}"
...
"$python_bin" -u tools/exp05_1_attention_test.py \
    --weights "$weights" \
    --data "$data_yaml" \
    --eval-root "$eval_root" \
    --run-name "$run_name" \
