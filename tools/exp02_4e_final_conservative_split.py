...
def main() -> None:
    ...
    # Exp02.4e：
    # 经过两轮人工复核，当前数据集中已检查的全部
    # dHash<=2 候选均属于真实近重复或相同拍摄场景。
    #
    # 因此只使用严格阈值 2 作为额外的组间约束。
    # 不恢复 Exp02.4b-v1 中更宽松的阈值 4/12，
    # 以避免相似关系传递造成大规模过度聚类。
    global_dhash_candidate_pairs = 0
    global_dhash_union_operations = 0
    ...
    for first_index in range(len(records)):
        ...
        for second_index in range(
            ...
            global_dhash_candidate_pairs += 1

            second_group_index = group_index[
                second["group_id"]
            ]

            root_before_first = union_find.find(
                first_group_index
            )

            root_before_second = union_find.find(
                second_group_index
            )
            ...
            if (
                ...
                global_dhash_union_operations += 1
    ...
    if max_group_ratio > 0.35:
        raise RuntimeError(
            "Conservative grouping produced a component "
            "larger than 35 percent of the dataset: "
            f"size={max_group_size}, "
            f"ratio={max_group_ratio:.6f}. "
            "The dominant scene should be downsampled "
    ...
    metadata = {
        ...
        "global_dhash_threshold": (
            AUDIT_DHASH_THRESHOLD
        ),
        "global_dhash_candidate_pairs": (
            global_dhash_candidate_pairs
        ),
        "global_dhash_union_operations": (
            global_dhash_union_operations
        ),
    ...
    print(
        " Exp02.4e Final Conservative Split Summary"
    )
    print(
        "============================================================"
    )
    ...
    print(
        "global_dhash_threshold="
        f"{AUDIT_DHASH_THRESHOLD}"
    )
    print(
        "global_dhash_candidate_pairs="
        f"{global_dhash_candidate_pairs}"
    )
    print(
        "global_dhash_union_operations="
        f"{global_dhash_union_operations}"
    )
    print(
    ...
    print(
        "exp02_4e_command_completed=YES"
    )
