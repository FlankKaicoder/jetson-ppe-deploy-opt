...
def convert_csv_value(value: str | None) -> Any:
    if value is None:
        return None
...
def load_results_rows(
    ...
    with results_csv.open(
        ...
        rows = [
            {
                key.strip(): convert_csv_value(value)
    ...
    if not rows:
        raise RuntimeError(
            "results.csv contains no training rows"
        )
...
def best_row(
    ...
    if not valid_rows:
        raise RuntimeError(
            f"no valid rows for metric: {metric_key}"
        )
        ...
        per_tensor.append(
            {
                ...
                "max_abs_error": tensor_maximum,
                "mean_abs_error": tensor_mean,
            }
        )
...
def main() -> int:
    ...
    best_map50_row = best_row(
        rows,
        "metrics/mAP50(B)",
    )

    best_map5095_row = best_row(
        rows,
        "metrics/mAP50-95(B)",
    )
    ...
    # Strict CPU FP32 conversion replay.
    ...
    with torch.inference_mode():
        training_output = reloaded_model(
            input_tensor
        )
    ...
    conversion_error = compare_outputs(
        training_output,
        deploy_output,
    )
    ...
    checks = {
        ...
        "best_pt_generated": (
            required_artifacts["best_pt"]
        ),
        "last_pt_generated": (
            required_artifacts["last_pt"]
        ),
    ...
    payload = {
        ...
        "final_row": selected_metrics(
            final_row
        ),
        "best_map50_row": selected_metrics(
            best_map50_row
        ),
        "best_map5095_row": (
            selected_metrics(
    ...
    lines = [
        ...
        (
            "pretrained_parameter_count="
            f"{pretrained_parameter_count}"
        ),
        (
            "trainer_parameter_count="
            f"{trainer_parameter_count}"
        ),
        (
            "trainer_rep_count="
            f"{trainer_rep_count}"
    ...
    for item in checkpoint_block_details:
        lines.extend(
            [
                ...
                (
                    "branch_1x1_gamma_abs_mean="
                    f"{item.get('branch_1x1_bn_gamma_abs_mean', 'NA')}"
                ),
                (
    ...
    for key, value in selected_metrics(
        final_row
    ).items():
        lines.append(
            f"{key}={value}"
