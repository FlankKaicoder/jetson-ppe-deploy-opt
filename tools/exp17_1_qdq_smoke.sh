...
run_name="exp17_1_qdq_smoke_${timestamp}"
...
qdq_onnx="${artifact_dir}/yolo11n_exp17_qdq_smoke.onnx"
engine="${artifact_dir}/yolo11n_exp17_qdq_smoke.engine"
...
python3 "${repo_root}/tools/exp17_make_qdq.py" \
    ...
    --limit 8 --calibration-chunk-size 8 --calibration-method entropy \
    > "${report_dir}/quantize.log" 2>&1
quantize_rc=$?
set -e
printf '%s\n' "${quantize_rc}" > "${report_dir}/quantize_return_code.txt"
[[ "${quantize_rc}" -eq 0 ]] || fail "QDQ quantization failed with ${quantize_rc}"
[[ -s "${qdq_onnx}" ]] || fail "QDQ ONNX missing"
...
[[ "${build_rc}" -eq 0 ]] || fail "TensorRT QDQ build failed with ${build_rc}"
[[ -s "${engine}" ]] || fail "QDQ Engine missing"
...
[[ "${execution_rc}" -eq 0 ]] || fail "QDQ execution smoke failed with ${execution_rc}"
...
printf '0\n' > "${report_dir}/return_code.txt"
echo "exp17_1_qdq_smoke=PASS output=${report_dir} artifact=${artifact_dir}"
